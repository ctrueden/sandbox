...
public class JavaFXKeys {
	...
	public static void main(final String... args) {
		...
		for (String s : accel) {
			KeyStroke ks = KeyStroke.getKeyStroke(s.replaceAll("\\^", "meta "));

			ModifierValue shift = ModifierValue.UP;
			ModifierValue control = ModifierValue.UP;
			ModifierValue alt = ModifierValue.UP;
			ModifierValue meta = ModifierValue.UP;
			ModifierValue shortcut = ModifierValue.UP;
			s = s.replaceAll("\\^", "shortcut ");
			String key = null;
			for (final String token : s.split(" ")) {
				switch (token) {
					case "shift": shift = ModifierValue.DOWN; break;
					case "control": control = ModifierValue.DOWN; break;
					case "ctrl": control = ModifierValue.DOWN; break;
					case "alt": alt = ModifierValue.DOWN; break;
					case "meta": meta = ModifierValue.DOWN; break;
					case "shortcut": shortcut = ModifierValue.DOWN; break;
					default: key = token; break;
				}
			}
			org.scijava.input.KeyCode sjKeyCode = org.scijava.input.KeyCode.get(key);
			if (sjKeyCode == org.scijava.input.KeyCode.UNDEFINED) {
	...
	private static KeyCode convertKeyCode(org.scijava.input.KeyCode sciJavaKeyCode) {
		switch (sciJavaKeyCode) {
			case A:
				return KeyCode.A;
			case ACCEPT:
